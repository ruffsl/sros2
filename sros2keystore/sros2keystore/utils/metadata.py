...
METADATA_DIR_NAME = '.sros_keystore'

METADATA_README_TEXT = """\
# SROS2 Keystore Metadata
This directory was generated by sros2 keystore and it contains persistent
configuration information used by the `keystore` command and its sub-commands.
"""
...
def get_metadata_root_path(workspace_path):
    ...
    :param workspace_path: The exact path to the root of a keystore workspace
...
def get_paths(workspace_path, profile_name, verb=None):
    ...
    :param workspace_path: The path to the root of a keystore workspace
    :type workspace_path: str
    :param profile_name: The keystore metadata profile name
    :type profile_name: str
    :param verb: (optional) The keystore verb with which this information is associated.
    ...
    metadata_path = os.path.join(
        profiles_path, profile_name) if profile_name else None

    # Get the metadata for this verb
    metadata_file_path = os.path.join(
        metadata_path, '%s.yaml' % verb) if profile_name and verb else None
...
def find_enclosing_workspace(search_start_path):
    ...
    Find a keystore workspace based on the existence of a sros_keystore
    ...
    :search_start_path: Directory which either is a keystore workspace or is
    contained in a keystore workspace
...
def migrate_metadata(workspace_path):
    ...
    last_version = None
    current_version = pkg_resources.require("sros2keystore")[0].version
...
def init_metadata_root(workspace_path, reset=False):
    """Create or reset a keystore metadata directory with no content in a given path.
    ...
    if not os.path.exists(workspace_path):
        raise IOError(
            "Can't initialize keystore workspace in path %s because it does "
    ...
    if marked_workspace and marked_workspace != workspace_path:
        raise IOError(
            "Can't initialize keystore workspace in path %s because it is "
    ...
    if os.path.exists(metadata_root_path):
        ...
        if reset:
            print("Deleting existing metadata from "
                  "keystore metadata directory: %s" % (metadata_root_path))
            shutil.rmtree(metadata_root_path)
            os.mkdir(metadata_root_path)
    else:
        # Create a new .sros_keystore directory
        os.mkdir(metadata_root_path)
...
def init_profile(workspace_path, profile_name, reset=False):
    ...
    :param workspace_path: The exact path to the root of a keystore workspace
    :type workspace_path: str
    :param profile_name: The keystore metadata profile name to initialize
    :type profile_name: str
    """
    ...
    if os.path.exists(profile_path):
        # Reset the directory if requested
        if reset:
            print("Deleting existing profile from "
                  "keystore profile directory: %s" % (profile_path))
...
def remove_profile(workspace_path, profile_name):
    ...
    :param workspace_path: The exact path to the root of a keystore workspace
    :type workspace_path: str
    :param profile_name: The keystore metadata profile name to delete
    :type profile_name: str
    """
    migrate_metadata(workspace_path)
...
def set_active_profile(workspace_path, profile_name):
    ...
    :param workspace_path: The exact path to the root of a keystore workspace
    :type workspace_path: str
    :param profile_name: The keystore metadata profile name to activate
    :type profile_name: str
    """
...
def get_metadata(workspace_path, profile, verb):
    ...
    :param workspace_path: The exact path to the root of a keystore workspace
    :type workspace_path: str
    :param profile: The keystore metadata profile name
    :type profile: str
    :param verb: The keystore verb with which this information is associated
    :type verb: str
...
def update_metadata(workspace_path, profile, verb, new_data={}, no_init=False, merge=True):
    """Update the keystore verb metadata for a given profile.
...
def update_active_metadata(workspace_path, verb, new_data={}):
    """Update the keystore verb metadata for the active profile.
